...
#read the data in the radar file
...
"""
Use lidar file to multiply modified radar file to filter out those are not ice
"""
...
iceburg using Ice_mass list. For example the iceburg1, the ice value is from 94500-96300, so set the treshold 
to pick the value up and sum them to get total mass of iceburg1. The mass of 5 iceburgs are
calculated below:
"""
...
def dragable(icemass):
    """
    If the iceburg mass is bigger than 36000000 is unable to drag, so
    create this function to check whether 5 iceburgs can be dragged
